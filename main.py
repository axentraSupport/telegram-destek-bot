...
# SENİN VERDİĞİN ÖZEL ÖDEME BİLGİLERİ
...
def callback_logic(call):
    ...
    u_data = db_query("SELECT balance, daily_claim, xp, level FROM users WHERE id=?", (uid,), fetch=True)[0]
    ...
    if call.data == "home":
        ...
        if u_data[0] >= 10:
            ...
            else: bot.send_message(uid, "😢 Şansına küs kanka, tekrar dene!")
        ...
        leaders = db_query("SELECT name, balance FROM users ORDER BY balance DESC LIMIT 5", fetch=True)
        text = "🏆 **ZENGİNLER LİSTESİ**\n\n"
        for i, l in enumerate(leaders): text += f"{i+1}. {l[0]} - {l[1]}₺\n"
        bot.edit_message_text(text, uid, mid, reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("⬅️ GERİ", callback_data="home")))

    elif call.data == "inv":
        items = db_query("SELECT item_name, date FROM inventory WHERE uid=?", (uid,), fetch=True)
        text = "📦 **ENVANTERİM**\n\n"
        if not items: text += "Henüz bir şey almamışsın kanka."
        for it in items: text += f"• {it[0]} ({it[1]})\n"
        bot.edit_message_text(text, uid, mid, reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("⬅️ GERİ", callback_data="home")))

    elif call.data == "admin_panel" and uid == ADMIN_ID:
        stats = db_query("SELECT COUNT(*), SUM(balance) FROM users", fetch=True)[0]
        bot.edit_message_text(f"👑 **ADMİN PANELİ**\n\n👥 Üye: {stats[0]}\n💰 Toplam Kasa: {stats[1]}₺", uid, mid, 
                             reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("⬅️ GERİ", callback_data="home")))

    elif call.data == "ticket":
        bot.edit_message_text("🛠️ **CANLI DESTEK**\n\nSorun varsa hemen yaz kanka: @axentradestek", uid, mid, 
                             reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("⬅️ GERİ", callback_data="home")))
...
def handle_receipt(message):
    ...
    if db_query("SELECT * FROM vault WHERE dna_hash=?", (dna,), fetch=True):
        bot.reply_to(message, "🚨 Bu dekont daha önce kullanılmış kanka!")
    else:
        ...
        bot.reply_to(message, "⏳ Dekontun iletildi patrona, onaylayınca bakiyen yüklenir.")

if __name__ == "__main__":
    keep_alive()
    # Conflict hatasını önlemek için infinity_polling ayarı
