...
# DB ÇAKIŞMASINI ÖNLEMEK İÇİN SIFIR DOSYA ADI
DB_NAME = "axentra_empire_v28.db"
bot = telebot.TeleBot(TOKEN, parse_mode="Markdown")

# --- [ 🗄️ VERİTABANI MOTORU - TÜM ÖZELLİKLERİ DESTEKLER ] ---
...
# Veritabanı Mimarisi (XP, Level, Spent, Ref hepsi içinde)
...
# --- [ 📱 ANA MENÜ - GÖRSELDEKİ DİZİLİM + EKSTRA ÖZELLİKLER ] ---
def get_main_menu(uid):
    ...
    # Alt Bilgi Çubuğu (Görseldeki XP/Level yapısı)
...
# --- [ ⚙️ ANA CALLBACK HANDLER - TÜM BUTONLARIN BEYNİ ] ---
...
def master_callback_router(call):
    ...
    # [MARKET VE OTOMATİK VIP (380₺ Kuralı)]
    ...
    # [OYUNLAR: SLOT, ÇARK, YAZI-TURA]
        ...
        if u['balance'] >= 10:
            ...
            if dice.dice.value in [1, 22, 43, 64]:
                ...
                bot.send_message(uid, f"🎊 **KAZANDIN! +{kazanc}₺ Hesaba Eklendi.**")
        ...
        if u['balance'] >= 25:
            ...
            elif win == "300 XP": db_query("UPDATE users SET xp = xp + 300 WHERE id=?", (uid,))
...
def start_handler(message):
    ...
    u = db_query("SELECT status FROM users WHERE id=?", (uid,), fetch=True)[0]
    bot.send_message(uid, f"🔱 **{MARKA_ADI} HOŞGELDİN**\n\n👤 **ÜYE SEVİYESİ = {u['status']}**", reply_markup=get_main_menu(uid))
...
def handle_receipt(message):
    ...
    bot.send_message(ADMIN_ID, f"🕵️ YENİ DEKONT\nKullanıcı ID: `{message.chat.id}`", reply_markup=mk)
...
def admin_confirm_payment(call):
    ...
    # 350₺ Bakiye Ekleme & Harcama Statüsü Güncelleme
    new_spent = u['spent'] + 350
    ...
    # [35₺ REFERANS BONUSU TETİKLEYİCİ]
    if u['ref_by'] != 0:
        db_query("UPDATE users SET balance = balance + 35 WHERE id=?", (u['ref_by'],))
        try: bot.send_message(u['ref_by'], "🎊 **Ref bonusun (35₺) yattı!** Davet ettiğin arkadaşın bakiye yükledi.")
        except: pass
    
    bot.send_message(tid, f"✅ **ÖDEMENİZ ONAYLANDI!**\n💰 Bakiyeniz eklendi.\n🛡️ Mevcut Statü: {new_status}")
